...
contract BabySafeMoon is Context, Ownable, IERC20 {
    ...
    uint256 public _giveawaysFee = 20;
    uint256 public _iBNBPoolFee = 30;
    uint256 public swapFee = _devFee.add(_marketingFee).add(_giveawaysFee).add(_iBNBPoolFee);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tSwap, uint256 tBurn) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tSwap, tBurn, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tSwap, tBurn);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tSwap).sub(tBurn);
        return (tTransferAmount, tFee, tSwap, tBurn);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tSwap, uint256 tBurn, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rSwap = tSwap.mul(currentRate);
		uint256 rBurn = tBurn.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rSwap).sub(rBurn);
	...
	function calculateBurnFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_burnFee).div(10**3);
           ...
           /// Setters functions  ///
          //////////////////////////
   function setBurnWallet(address _address) external onlyOwner returns (bool){
    ...
    function setFees(uint256 taxFee, uint256 burnFee, uint256 marketingFee, uint256 giveawaysFee, uint256 iBNBPoolFee) external onlyOwner returns (bool){
        _taxFee = taxFee;
        _burnFee = burnFee;
        _marketingFee = marketingFee;
        _giveawaysFee = giveawaysFee;
        _iBNBPoolFee = iBNBPoolFee;
        swapFee = _devFee.add(_marketingFee).add(_giveawaysFee).add(_iBNBPoolFee);
        return true;
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 && swapFee == 0) return;
    ...
    function restoreAllFee() private {
		...
		_giveawaysFee = _previousGiveawaysFee;
		_iBNBPoolFee = _previousIBNBPoolFee;
    }
    ...
    function swapAndSendToFees(uint256 tokens) private lockTheSwap{
        ...
        payable(marketingWallet).transfer(transferBalance.div(swapFee).mul(_marketingFee));
        payable(devWallet).transfer(transferBalance.div(swapFee).mul(_devFee));
        payable(giveawaysWallet).transfer(transferBalance.div(swapFee).mul(_giveawaysFee));
        payable(iBNBPool).transfer(transferBalance.div(swapFee).mul(_iBNBPoolFee));
    }
   ...
   function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tSwap, uint256 tBurn) = _getValues(tAmount);
